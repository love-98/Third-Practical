       ...
       while($row = mysqli_fetch_assoc($getrowrsult)){
         if($row['PAGE_TITLE']!==""){?>
          <tr>            
            ...
            <?PHP $SRNO = $row['SR NO'];?>
            <td><?PHP ECHO "<a href='updatedescription.php?SNO=$SRNO'>"; ?><button class="btn-primary rounded" > Edit </button></a> <?PHP ECHO "<a href='delete.php?sno=$SRNO'>"; ?><button class="BTN btn-primary  rounded"> Delete </button> </a></td>    
          </tr>    
          <?php }?>
   <?PHP } ?>
        </tbody>
